...
def RevisionCronograma(lista: list)->None:
    ...
    **Descripción:**
    ...
    for fila in range(1, len(lista)):
        if os.path.isfile(lista[fila][0]) == False:
            print(">> ERROR: archivo de Imagen no existe o ruta incorrecta.", file=sys.stderr)
    ...
    for fila in range(1, len(lista)):
        for col in range(1, 2):
            ...
            if valor == False:
                ...
                print("   Fila: {}, Columna: {}, Texto: {}".format(fila, col, lista[fila][col]), file=sys.stderr)
                print("   Programa terminado.", file=sys.stderr)
                exit(1)
